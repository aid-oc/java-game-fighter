package scripts.MassFighterTest.ProfileTasks;
...
public class HillGiantsRoute extends Task{
    ...
    public boolean validate() {
        return (Inventory.isFull() || (Inventory.containsAnyOf(MassFighterTest.combatProfile.getLootNames()) && !MassFighterTest.methods.inFightAreas(Players.getLocal()))) || !Inventory.containsAnyOf(MassFighterTest.combatProfile.getLootNames()) && !MassFighterTest.methods.inFightAreas(Players.getLocal());
    }

    @Override
    public void execute() {
        MassFighterTest.status = "Hill Giants: Following Route";
        if (Inventory.contains("Brass key")) {

            final Area fightArea = MassFighterTest.combatProfile.getFightAreas().get(0);
            final BankingProfile bankProfile = (BankingProfile) MassFighterTest.combatProfile;
            ...
            } else if (Inventory.containsAnyOf(MassFighterTest.combatProfile.getLootNames()) && bankArea.contains(player)) {
                        ...
                        if (Bank.open()) {
                            Bank.depositAllExcept(new Filter<SpriteItem>() {
                                ...
                                public boolean accepts(SpriteItem spriteItem) {
                                    ...
                                    return (MassFighterTest.food != null && (MassFighterTest.food.getName().equals(name)) || name.equals("Brass key"));
                                }
                            });
                            if (MassFighterTest.useFood && MassFighterTest.food != null && Inventory.getQuantity(MassFighterTest.food.getName()) < 10) {
                                System.out.println("Withdrawing food");
                                if (Bank.withdraw(MassFighterTest.food.getName(), 10)) {
                                    Execution.delayUntil(() -> Inventory.contains(MassFighterTest.food.getName()));
                                }
                            }
                            Bank.close();
                        }
            ...
            } else if (!Inventory.containsAnyOf(MassFighterTest.combatProfile.getLootNames()) && !fightArea.contains(player)) {
            ...
            JOptionPane.showMessageDialog(null, "You need a Brass key to run this profile", "MassFighterTest", JOptionPane.WARNING_MESSAGE);
        }
    }
}
